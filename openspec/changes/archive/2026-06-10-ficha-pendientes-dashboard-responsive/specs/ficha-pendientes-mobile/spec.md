...
(Previously: ficha mobile used separate badge blocks inside nested ficha cards.)
...
(Previously: ficha-specific nested cards only approximated dashboard mobile styling.)
...
(Previously: modal behavior was specified only for tapping mobile badges.)
...
(Previously: desktop preserved the original ficha-specific Concepto/Revisar layout.)
...
(Reason: ficha pendientes now use dashboard/index-style cobro concept buttons and the lightweight `#modalCobro` flow across desktop and mobile.)
