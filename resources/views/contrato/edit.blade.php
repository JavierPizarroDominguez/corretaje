    ...
    <div class="card">
        ...
        <div class="mb-3">
            ...
            <select name="administracion" class="form-select">
                <option value="">— Seleccionar —</option>
                <option value="1" {{ old('administracion', $contrato->administracion) == '1' ? 'selected' : '' }}>Sí</option>
                <option value="0" {{ old('administracion', $contrato->administracion) == '0' ? 'selected' : '' }}>No</option>
            </select>
            @error('administracion') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Comision Inicial</label>
            <input type="number" name="comision_inicial" class="form-control" value="{{ old('comision_inicial', $contrato->comision_inicial) }}">
            @error('comision_inicial') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Garantia</label>
            <input type="number" name="garantia" class="form-control" value="{{ old('garantia', $contrato->garantia) }}">
            @error('garantia') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Renta</label>
            <input type="number" name="renta" class="form-control" value="{{ old('renta', $contrato->renta) }}">
            @error('renta') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Dia Pago</label>
            <input type="number" name="dia_pago" class="form-control" value="{{ old('dia_pago', $contrato->dia_pago) }}">
            @error('dia_pago') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Comision Mensual</label>
            <input type="number" name="comision_mensual" class="form-control" value="{{ old('comision_mensual', $contrato->comision_mensual) }}">
            @error('comision_mensual') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Fecha Firma</label>
            <input type="datetime-local" name="fecha_firma" class="form-control" value="{{ old('fecha_firma', $contrato->fecha_firma) }}">
            @error('fecha_firma') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Fecha Inicio</label>
            <input type="datetime-local" name="fecha_inicio" class="form-control" value="{{ old('fecha_inicio', $contrato->fecha_inicio) }}">
            @error('fecha_inicio') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Fecha Termino</label>
            <input type="datetime-local" name="fecha_termino" class="form-control" value="{{ old('fecha_termino', $contrato->fecha_termino) }}">
            @error('fecha_termino') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Url Pdf</label>
            <input type="text" name="url_pdf" class="form-control" value="{{ old('url_pdf', $contrato->url_pdf) }}">
        ...
        <div class="mb-3">
            ...
            @endif
                ...
                <div class="d-flex gap-2 mt-4 pt-3 border-top">
                    ...
                    <a href="/contrato/{{ $contrato->id }}" class="btn btn-outline-secondary btn-sm">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
