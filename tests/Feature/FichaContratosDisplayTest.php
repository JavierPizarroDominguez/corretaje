...
class FichaContratosDisplayTest extends TestCase
{
    ...
    public function test_propiedad_contratos_renders_termination_preview_modal_with_pending_and_default_rows(): void
    {
        ...
        $response->assertSee('terminacion-warning', false);
        $response->assertSee('devolución de garantía quedará pendiente');
        $response->assertSee('30 días');
        $response->assertSee('cobros proporcionales de renta');
        $response->assertSee('Devolver garantía');
        $response->assertDontSee('Agregar cobro');
        $response->assertDontSee('Aseo final');
        $response->assertDontSee('Extra');
        $response->assertDontSee('terminacion-ajuste', false);
        $response->assertDontSee('terminacion-validation-error', false);
    ...
    public function test_cliente_contratos_termination_preview_modal_keeps_empty_pending_state_and_no_persistence_surface(): void
    {
        ...
        $response->assertSee('Al terminar el contrato se fijará la fecha de término');
        $response->assertSee('devolución de garantía quedará pendiente');
        $response->assertDontSee('Agregar cobro');
        $response->assertDontSee('Aseo final');
        $response->assertDontSee('Extra');
        ...
        $this->assertStringNotContainsString('terminacion-description', $component);
    ...
    public function test_termination_modal_confirm_uses_fetch_loading_modal_feedback_and_frontend_ceiling_validation(): void
    {
        ...
        $this->assertStringNotContainsString('validateTerminationDiscounts(preview)', $component);
        $this->assertStringNotContainsString('Los descuentos no pueden superar la garantía.', $component);
        ...
        $this->assertStringNotContainsString('descuentos:', $component);
